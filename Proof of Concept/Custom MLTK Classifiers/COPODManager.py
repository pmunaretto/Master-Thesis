...
from pyod.models.copod import COPOD as _COPOD
...
from algos_contrib.COPOD import COPOD
...
class COPODManager(ClustererMixin, BaseAlgo):
    ...
    def fit(self, df, options):
        ...
        COPOD.register_codecs()

        # Remove all parameters that are destined for the manager class
        copod_options = options.copy()
        copod_options["params"] = {k: v for k, v in options["params"].items() if not k.startswith("mgr_")}
        ...
        for group_name, group_df in grouped:
            ...
            # Train the COPOD classifier on the group dataframe and optionally save it
            clf = COPOD(options=copod_options)
            results = clf.fit(df=group_df, options=copod_options)
            all_results = pd.concat([all_results, results])
            if self.out_params["mgr_save"]:
                save_model(clf, model_dir, group_name, copod_options)
...
def save_model(clf, model_dir, name, options):
    ...
    with open(file_path, mode="w") as f:
        ...
        model_writer.writerow(["COPOD", opaque, json.dumps(options)])
